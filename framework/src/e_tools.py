...
def train_identity_map_with_emb(D, H, E, sampler, batch_size=1024, max_iter=5000, lr=1e-3, tol=1e-3, blow=3, verbose=True, emb_method = 'transformer'):
...
def train_gaussians_identity_map_with_emb(D, H, E, distributions, batch_size=1024, max_iter=5000, lr=1e-3, tol=1e-3, blow=3, verbose=False, emb_method = 'transformer'):
    "Trains potential D to satisfy D.push(x)=x by using MSE loss w.r.t. sampler's distribution"
    unfreeze(H)
    unfreeze(E)
    opt_e = torch.optim.Adam(E.parameters(), lr=lr, weight_decay=1e-10)
