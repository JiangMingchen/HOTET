...
class Wasserstein2BarycenterBenchmark(Benchmark):
    ...
    def _compute_gaussian_barycenter(self, max_iter=1000, tol=1e-6):
        ...
        for n in tqdm(range(self.num)) if self.verbose else range(self.num):
            ...
            map_inv.weight.data = torch.tensor(weight_inv, dtype=torch.float32, device=self.device)
            map_inv.bias.data = torch.tensor(bias_inv, dtype=torch.float32, device=self.device)
            ...
            map_fwd.weight.data = torch.tensor(weight, dtype=torch.float32, device=self.device)
            map_fwd.bias.data = torch.tensor(bias, dtype=torch.float32, device=self.device)
