...
def train_gaussians_identity_map(D, X_points, batch_size=1024, max_iter=5, lr=1e-3, tol=1e-3, blow=3, convex=False, verbose=False):
    "Trains potential D to satisfy D.push(x)=x by using MSE loss w.r.t. sampler's distribution"
    unfreeze(D)
    opt = torch.optim.Adam(D.parameters(), lr=lr, weight_decay=1e-10)
    if verbose:
    ...
    for iteration in tqdm(range(max_iter)) if verbose else range(max_iter):
        ...
        indices = np.random.choice(len(X_points), batch_size, replace=True)
...
def train_wishart_identity_map(D, dist_pair, batch_size=1024, max_iter=5, lr=1e-3, tol=1e-3, blow=3, convex=False, verbose=False):
    "Trains potential D to satisfy D.push(x)=x by using MSE loss w.r.t. sampler's distribution"
    unfreeze(D)
    opt = torch.optim.Adam(D.parameters(), lr=lr, weight_decay=1e-10)
    if verbose:
    ...
    for iteration in tqdm(range(max_iter)) if verbose else range(max_iter):
        # X = torch.tensor(np.random.multivariate_normal(mean, cov, batch_size), dtype=torch.float32).cuda()
        X, _ = dist_pair.sample(batch_size=batch_size)
        with torch.no_grad():
            X *= blow
        X.requires_grad_(True)
...
def train_wishart_identity_map_with_emb(D, E, H, dist_pair, batch_size=1024, max_iter=5, lr=1e-3, tol=1e-3, blow=3, convex=False, verbose=False, emb_method='transformer'):
    unfreeze(H)
    unfreeze(E)
    opt_e = torch.optim.Adam(E.parameters(), lr=lr, weight_decay=1e-10)
    opt = torch.optim.Adam(H.parameters(), lr=lr, weight_decay=1e-10)
    if verbose:
        print('Training the potentials to satisfy push(x)=x')
    pbar = tqdm(range(max_iter))
    W = None
    for iteration in pbar if verbose else range(max_iter):
        X, _ = dist_pair.sample(batch_size=batch_size)
        with torch.no_grad():
            X *= blow
        X.requires_grad_(True)
        emb_X_batch = E.forward(x=X, method=emb_method)
        # W = H.forward(cond_input=emb_X_batch)
        W = H.forward(embedding=emb_X_batch)
        loss = F.mse_loss(D.push(X, W), X.detach())
        loss.backward()
        opt.step(); opt.zero_grad()
        opt_e.step(); opt_e.zero_grad()

        pbar.set_description(f'loss={loss.item():.4f}, tol={tol:.4f}')
        if loss.item() < tol:
            break
